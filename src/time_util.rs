...
const PARSE_FNS: [DateFn; 3] = [
  |s: &str| FixedOffset::east(0).datetime_from_str(s, "%Y-%m-%dT%H:%M:%S%.fZ"),
  |s: &str| FixedOffset::east(0).datetime_from_str(s, "%Y-%m-%dT%H:%M:%SZ"),
  |s: &str| DateTime::parse_from_str(s, "%Y-%m-%dT%H:%M:%S%.f%z"),
];
